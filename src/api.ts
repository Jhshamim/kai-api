...
apiRouter.use(cors());
  ...
  apiRouter.get("/search", async (req, res) => {
    try {
      ...
      const filters: scraper.SearchFilters = {
        ...
        // Helper to convert single string to array if only one is passed
  ...
  apiRouter.get("/episodes", async (req, res) => {
    try {
      ...
      if (!aniId) return res.status(400).json({ status: 400, success: false, error: "Missing aniId parameter. Get it from /details." });
  ...
  apiRouter.get("/stream", async (req, res) => {
    try {
      ...
      const server = req.query.server as string;   // hd-1 or hd-2
      const type = req.query.type as string;       // dub or sub
  ...
  // Example for a general raw scraper:
  apiRouter.get("/raw", async (req, res) => {
    try {
      res.json({ message: "Not implemented. Use specific endpoints." });
    } catch (e: any) {
